...
const test = () => {
    ...
    const positions = [-30, -200, -420, -680, -910, -1155];
    let counter = 1;
    //slider.style.transform = 'translateX(-1155vw)';
    setInterval(() => {
        slider.style.transform = 'translateX(' + positions[counter] + 'vw)';
        console.log('set');
        counter = counter !== 5 ? counter += 1 : counter = 0;
    }, 3000);
}
test();
//show();
function show() {
    ...
    t.fromTo(img, time, { opacity: '0' }, { opacity: '1' })
        ...
        .to(img, time, { opacity: '1' })
        .to(img, time, { opacity: '0', onComplete: () => img.src = './assets/img/arr2/img1.jpg' }, '+=1')
        .to(img, time, { opacity: '1' })
        .to(img, time, { opacity: '0', onComplete: () => img.src = './assets/img/arr2/img8.jpg' }, '+=1')
        .to(img, time, { opacity: '1' })
        ...
        .to(img, time, { opacity: '1' })
        .to(img, time, { opacity: '0', onComplete: () => img.src = './assets/img/rose.png' },'+=1')
